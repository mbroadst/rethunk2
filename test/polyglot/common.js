...
// ========== Start of common test functions ==========


// Tests polyglot/default.js7.2 (JavaScript 7.2): Tests r.default (/Users/mbroadst/Development/rethinkdb/test/rql_test/src/default.yaml)
...
var tables_to_cleanup = [ ['rethinkdb', '_debug_scratch'] ] // pre-existing tables
...
function eq(exp, compOpts) {
  ...
  fun.hasDesc = true;
  fun.toString = function() { return JSON.stringify(exp); }
...
function returnTrue() {
  ...
  fun.hasDesc = true;
  fun.toString = function() { return 'Always true'; }
  fun.toJSON = fun.toString;
  return fun;
}
...
function atexitCleanup(exitCode) {
  if (!__conn_cache['admin']) {
    ...
    if (promisesToKeep.length > 0) {
      promise.all(promisesToKeep).then(function () { process.exit(exitCode); });
    } else {
  ...
  try {
    ...
    // - pull out feeds and cursors to arrays
      ...
      if (!Number.isNaN(Number(test.testopts.result_limit)) && test.testopts.result_limit > 0) {
        ...
        result.each(function processResult_iter(err, row) {
          if (err) {
...
function uuid() {
  ...
  fun.toString = function() { return 'uuid()'; };
  fun.toJSON = fun.toString;
  return fun;
}
...
module.exports = {
  ...
  setup_table_check: setup_table_check,
  test: test,
  define: define,
  the_end: the_end
};
