...
Connection.prototype._processResponse = function(response, token) {
  if (this.outstandingCallbacks[token] === undefined) {
    return;
  }

  let metadata = this.outstandingCallbacks[token];
  ...
  let opts = metadata.opts;
  let profile = response.p;
  ...
  case protoResponseType.RUNTIME_ERROR:
    let errType = util.errorClass(response.e);
  ...
  case protoResponseType.SUCCESS_PARTIAL:
    ...
    for (let k = 0, len1 = ref1.length; k < len1; k++) {
      let note = ref1[k];
      if (note === protodef.Response.ResponseNote.SEQUENCE_FEED) {
        if (cursor == null) {
          cursor = new cursors.Feed(this, token, opts, root);
        }
      } else if (note === protodef.Response.ResponseNote.UNIONED_FEED) {
        if (cursor == null) {
          cursor = new cursors.UnionedFeed(this, token, opts, root);
        }
      } else if (note === protodef.Response.ResponseNote.ATOM_FEED) {
        if (cursor == null) {
          cursor = new cursors.AtomFeed(this, token, opts, root);
        }
      } else if (note === protodef.Response.ResponseNote.ORDER_BY_LIMIT_FEED) {
        if (cursor == null) {
  ...
  case protoResponseType.SERVER_INFO:
    ...
    return;
  default:
    cb(new errors.ReqlDriverError('Unknown response type'));
  }
};
...
Connection.prototype._continueQuery = function(token) {
  ...
  return this._sendQuery({ type: protoQueryType.CONTINUE, token: token });
};
...
Connection.prototype._endQuery = function(token) {
  ...
  return this._sendQuery({ type: protoQueryType.STOP, token: token });
};
